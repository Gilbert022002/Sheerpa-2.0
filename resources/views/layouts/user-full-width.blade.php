...
<body class="bg-background-light text-text-main-light min-h-screen flex flex-col">
    ...
    <div class="flex flex-1 w-full max-w-[1400px] mx-auto">
        <main class="flex-1 p-6 md:p-8 space-y-6">
            {{-- Horizontal Navigation Bar - Same style as sidebar --}}
            <nav class="flex items-center justify-center gap-2 overflow-x-auto pb-2">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl whitespace-nowrap {{ request()->routeIs('dashboard') ? 'bg-primary/10 text-primary font-bold' : 'text-text-sub-light hover:bg-white hover:text-primary' }} transition-all">
                    <span class="material-symbols-outlined">dashboard</span> Dashboard
                </a>
                <a href="{{ route('user.profile') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl whitespace-nowrap {{ request()->routeIs('user.profile') ? 'bg-primary/10 text-primary font-bold' : 'text-text-sub-light hover:bg-white hover:text-primary' }} transition-all">
                    <span class="material-symbols-outlined">person</span> My Profile
                </a>
                <a href="{{ route('user.courses.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl whitespace-nowrap {{ request()->routeIs('user.courses.*') ? 'bg-primary/10 text-primary font-bold' : 'text-text-sub-light hover:bg-white hover:text-primary' }} transition-all">
                    <span class="material-symbols-outlined">school</span> Parcourir les cours
                </a>
                <a href="{{ route('user.favorites.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl whitespace-nowrap {{ request()->routeIs('user.favorites.*') ? 'bg-primary/10 text-primary font-bold' : 'text-text-sub-light hover:bg-white hover:text-primary' }} transition-all">
                    <span class="material-symbols-outlined">favorite</span> Mes Favoris
                </a>
                <a href="{{ route('user.bookings.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl whitespace-nowrap {{ request()->routeIs('user.bookings.*') ? 'bg-primary/10 text-primary font-bold' : 'text-text-sub-light hover:bg-white hover:text-primary' }} transition-all">
                    <span class="material-symbols-outlined">calendar_today</span> Mes Réservations
                </a>
                <a href="{{ route('user.tutors.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl whitespace-nowrap {{ request()->routeIs('user.tutors.*') ? 'bg-primary/10 text-primary font-bold' : 'text-text-sub-light hover:bg-white hover:text-primary' }} transition-all">
